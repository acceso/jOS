...
static u8 kma_blocksizes_idx(u8 power)
{
	s8 i = 0;
...
static void *kma_early_alloc_next_busy(void)
{
	s8 bit;
...
static void kma_early_free_busy(void *addr)
{
...
static inline void kma_free_page(void *addr)
{
	kma.npages--;
...
static u8 kma_pool_contraction(void)
{
	u8 i = 0;

	/* Can't use list_for_each because the list can be modified
	 * and don't have something like list_for_each_safe */
...
static void *kma_pool_expand_one(void)
{
	void *p;
...
static inline void kma_merge_next(struct list_head *which, u8 idx)
{
	struct list_head *n;
...
static inline u8 kma_try_merge_next(struct list_head *which, u8 idx)
{
 ...
 */
static inline void kma_try_merge(u8 idx)
{
	...
	if (idx == BLOCKSIZES_SZ - 1)
		 ...
		 * list_for_each_safe wouldn't work because two elements are
		 * removed. This should work without messing with the lists */
		list_for_each(c, &kma.free[idx].l) {
...
static inline void *give_first_and_split_buddies(u8 idx, u8 power)
{
...
static s8 kma_busy_cmp(const struct tnode *a, const struct tnode *b)
{
