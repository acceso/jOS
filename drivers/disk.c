...
static u8 ide_ready_wait(struct _drive *dev)
{
	u8 s;
	u16 times = 0;
...
static u8 ide_cmd_read(struct _drive *dev, u16 reg)
{
	ide_ready_wait(dev);
...
static u16 ide_cmd_read16(struct _drive *dev, u16 reg)
{
	ide_ready_wait(dev);
...
static void ide_cmd_write(struct _drive *dev, u16 reg, u8 data)
{
	ide_ready_wait(dev);
...
static void ide_cmd_write16(struct _drive *dev, u16 reg, u16 data)
{
	ide_ready_wait(dev);
...
static u8 ide_dev_sel(struct _drive *dev)
{
...
#if 0
static void ide_reset(struct _drive *dev)
{
	/* Not yet done... */
	return;
...
static void do_ide_dev(struct _drive *dev)
{
	/* Things to check:
...
__isr__ do_ide1(struct intr_frame r)
{
	intr_enter();
...
__isr__ do_ide2(struct intr_frame r)
{
	intr_enter();
...
void ide_strncpy(char *dest, char *src, s16 l)
{
	char *p;
	u8 saved_l;
...
static u8 ide_identify(struct _drive *dev)
{
...
static void read_pentry(struct _drive *dev, u8 pnum, u8 *first_sector)
{
	p_entry *p;
...
static void read_partitions(struct _drive *dev)
{
	u8 first_sector[512];
...
static void ide_init_dev(struct _channels *channel, u8 devnum)
{
	struct _drive *dev;
...
static void ide_init(u8 ch_num, u16 iobase, u8 irq, void *fp)
{
...
static inline void ide_prepare_rwop(struct _drive *dev, u8 cmd, u64 sector, size_t count)
{
	ide_dev_sel(dev);
...
struct _drive *ide_get_dev(dev_t *device)
{
	switch (device->minor) {
	case 0 ... 15:
...
static size_t ide_partition_pos(struct _drive *dev, u32 minor, size_t pos)
{
 ...
 * count: size of addr
 */
static size_t ide_read(dev_t *device, size_t pos, void *addr, size_t count)
{
	struct _drive *dev;
	size_t realpos;
...
static size_t ide_write(dev_t *device, size_t pos, void *addr, size_t count)
{
	struct _drive *dev;
	size_t realpos;
...
static size_t ide_open(dev_t *device)
{
	struct _drive *dev;
